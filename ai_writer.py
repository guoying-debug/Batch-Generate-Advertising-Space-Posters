...
请遵守以下要求：
1. 如果提供了参考图，必须优先继承参考图的版式语言、视觉节奏、配色关系、装饰元素和主体呈现方式；内容主题只负责替换主体与文案，不得无视参考图重做成另一种风格。
...
你必须只输出一个合法JSON对象，不要输出额外解释。JSON结构如下：
{
  ...
  "visual_strategy": {
    ...
    "template_id": "模板ID，如reference_editorial、reference_bold、reference_showcase、generic",
    "banner_layout_mode": "横幅版式建议",
    "square_layout_mode": "方图版式建议",
    "visual_language": "一句话描述视觉语言",
    "style_strength": "参考图风格控制强度，strong或medium",
    "decoration_density": "装饰密度，low、medium或high",
    "overlay_strength": "蒙版强度，light、medium或strong",
...
def analyze_reference_images(images: list) -> dict:
    ...
    content = [{
        ...
        "text": (
            ...
            "replaceable_slots、fixed_elements、prompt_boost、theme、template_id、layout_mode、"
            "banner_layout_mode、square_layout_mode、image_strategy、visual_language、style_strength、"
            "decoration_density、overlay_strength、layout_signature、layout_reason。"
            ...
            "layout_signature 是不超过20字的版式识别标签，例如 左文右图分栏、居中主视觉、上文下图卡片；"
            "layout_reason 是不超过40字的版式判断依据，说明为什么判成这个模板；"
            "template_id 只能在 reference_editorial、reference_bold、reference_showcase、generic 中选择；"
            "layout_mode、banner_layout_mode、square_layout_mode 只能在 left_text_right_visual、top_text_bottom_visual、centered 中选择；"
            "image_strategy 只能在 full_background、local_replace 中选择；"
            "style_strength 只能在 strong、medium 中选择；"
            "decoration_density 只能在 low、medium、high 中选择；"
            "overlay_strength 只能在 light、medium、strong 中选择；"
            "注意：版式判断优先级高于颜色判断。先看文字区与主体区的空间关系、信息块布局、卡片分区和视觉重心，再决定 template_id 与 layout_mode；"
            "如果颜色很像但版式不同，必须判成不同模板或不同版式；如果版式很像但颜色不同，优先保持同一模板。"
            "theme 是一个对象，包含从参考图提取的实际颜色值（十六进制），字段："
            "primary（主色/背景色）、accent（强调色/按钮色）、text（主文字色）、sub_text（副文字色）、"
            "card_bg（卡片/信息块背景色）。如果参考图颜色不明确则用 null。"
        ),
    }]
    for img in valid[:4]:
    ...
    try:
        ...
        return {
            ...
            "banner_layout_mode": data.get("banner_layout_mode", ""),
            "square_layout_mode": data.get("square_layout_mode", ""),
            "image_strategy": data.get("image_strategy", ""),
            "visual_language": data.get("visual_language", ""),
            "style_strength": data.get("style_strength", ""),
            "decoration_density": data.get("decoration_density", ""),
            "overlay_strength": data.get("overlay_strength", ""),
...
def _normalize_plan(plan: dict) -> dict:
    ...
    defaults = {
        ...
        "template_id": "generic",
        "visual_language": "以内容为主导的现代商业海报视觉",
...
def _merge_reference_analysis(plan: dict, reference_analysis: dict = None) -> dict:
    ...
    if ref.get("layout"):
        visual["composition"] = ref["layout"]
    if ref.get("style"):
        visual["style"] = ref["style"]
    if ref.get("color_palette"):
        visual["color_palette"] = ref["color_palette"]
    if ref.get("subject"):
        visual["subject"] = ref["subject"]
    if ref.get("template_id"):
        visual["template_id"] = ref["template_id"]
    if ref.get("layout_mode"):
        visual["layout_mode"] = ref["layout_mode"]
    if ref.get("banner_layout_mode"):
        visual["banner_layout_mode"] = ref["banner_layout_mode"]
    if ref.get("square_layout_mode"):
        visual["square_layout_mode"] = ref["square_layout_mode"]
    if ref.get("image_strategy"):
        visual["image_strategy"] = ref["image_strategy"]
    if ref.get("visual_language"):
        visual["visual_language"] = ref["visual_language"]
    if ref.get("style_strength"):
        visual["style_strength"] = ref["style_strength"]
    if ref.get("decoration_density"):
        visual["decoration_density"] = ref["decoration_density"]
    if ref.get("overlay_strength"):
        visual["overlay_strength"] = ref["overlay_strength"]
    if ref.get("theme"):
        visual["theme"] = ref["theme"]
    ...
    if boost:
        for item in plan.get("size_adaptations", []):
            ...
            if prompt and boost not in prompt:
                item["final_prompt"] = f"必须继承参考图风格特征：{boost}。{prompt}"[:760]
    return plan
...
def _fallback_plan(
    ...
    ref_template = (reference_analysis or {}).get("template_id") or "generic"
    ref_layout = (reference_analysis or {}).get("layout_mode") or "left_text_right_visual"
    ref_banner_layout = (reference_analysis or {}).get("banner_layout_mode") or ref_layout
    ref_square_layout = (reference_analysis or {}).get("square_layout_mode") or ref_layout
    ref_image_strategy = (reference_analysis or {}).get("image_strategy") or "full_background"
    ref_visual_language = (reference_analysis or {}).get("visual_language") or "以参考图主导的商业海报视觉"
    ref_style = (reference_analysis or {}).get("style") or "现代商业海报风格"
    ref_color = (reference_analysis or {}).get("color_palette") or "与参考图一致的主色和强调色关系"
    ref_overlay = (reference_analysis or {}).get("overlay_strength") or "medium"
    size_adaptations = []
    for size in sizes:
        if size == "2560x320":
            layout_mode = ref_banner_layout
            layout = "横向长条构图，画面节奏、文字与主体分区继承参考图"
        elif size == "1160x1016":
            layout_mode = ref_square_layout
            layout = "方图构图，视觉重心、留白位置与装饰关系继承参考图"
        else:
            layout_mode = ref_layout
            layout = "根据尺寸保持主体、主标题和按钮三者不冲突，同时延续参考图构图逻辑"
        ...
        final_prompt = (
            f"{main_prompt} 必须延续参考图的视觉语言：{ref_visual_language}。"
            f" 风格为{ref_style}，配色关系为{ref_color}，版式为{layout}。"
            "画面文字严格按以下顺序排列："
            f'左上角品牌小字"{brand}"，'
    ...
    return _merge_reference_analysis(_normalize_plan({
        ...
        "visual_strategy": {
            "scene": scene,
            "subject": subject,
            "composition": (reference_analysis or {}).get("layout") or "主体明确，文字与视觉分区清晰",
            "layout_mode": ref_layout,
            "banner_layout_mode": ref_banner_layout,
            "square_layout_mode": ref_square_layout,
            "image_strategy": ref_image_strategy,
            "template_id": ref_template,
            "visual_language": ref_visual_language,
            "style_strength": (reference_analysis or {}).get("style_strength") or "strong",
            "decoration_density": (reference_analysis or {}).get("decoration_density") or "medium",
            "overlay_strength": ref_overlay,
            "replaceable_slots": [
                "品牌文字位",
                "主标题位",
                "副标题位",
            ...
            "fixed_elements": list((reference_analysis or {}).get("fixed_elements") or ["参考图中的核心层级与装饰关系"]),
            "lighting": "柔和暖白或自然光的商业海报光线",
            "style": ref_style,
            "color_palette": ref_color,
            "texture_details": "强调材质、层次、细节和空间真实感",
            "text_safe_area": "文字区域跟随参考图留白关系，避免主体遮挡主标题和按钮",
        },
