...
def step_extract_and_write(url, operator_text):
    ...
    detail = ai_writer.generate_detail(content, operator_text)

    img_count = len(content.get("images", []))
    text_len = len(content.get("full_text", ""))
    info = (
        f"标题: {content['title']}\n"
        f"简介: {content['desc'][:60]}\n"
        f"封面: {content['cover'][:70]}\n"
        f"抓到正文 {text_len} 字 / 图片 {img_count} 张"
    )
    return (
        content["cover"],              # cover_state
        info,
        copy["title"],
        copy["subtitle"],
        copy["cta"],
        copy.get("image_prompt", ""),  # banner/方图生图描述
        detail,                        # detail_state（dict）
        detail.get("image_prompt", copy.get("image_prompt", "")),  # 详情页生图描述（可编辑）
    )


def step_make(cover_url, title, subtitle, cta, mode, upload_img, image_prompt,
              detail_state, detail_prompt):
    """③ 按模式合成 banner + 方图 + 详情页长图"""
    if mode == MODE1:
    ...
    # 详情页背景：优先用详情页专属描述（模式一）或相同 bg（模式二）
...
with gr.Blocks(title="酷家乐广告海报生成工具") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            info_box = gr.Textbox(label="提取内容", lines=4, interactive=False)
    ...
    extract_btn.click(
        ...
        outputs=[cover_state, info_box, title_box, subtitle_box, cta_box,
                 prompt_box, detail_state, detail_prompt_box],
    )
    make_btn.click(
        step_make,
        inputs=[cover_state, title_box, subtitle_box, cta_box, mode, upload_img,
                prompt_box, detail_state, detail_prompt_box],
        outputs=[banner_out, square_out, detail_out],
    )
